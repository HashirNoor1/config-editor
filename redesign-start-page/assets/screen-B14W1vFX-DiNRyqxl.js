import{V as e,_ as t,c as n,z as r}from"./main-B3cgxoEb.js";import{D as i,E as a,F as o,G as s,J as c,N as l,P as u,Q as d,T as f,W as p,dt as m,et as h,f as g,m as _,pt as v,t as y,ut as b,x}from"./vue.esm-bundler-DB3X0LGu-CA37-FN0.js";import{t as S}from"./vue-i18n-BPTPk0JE-atk3okNg.js";import{t as C}from"./marked.esm-BgD7BtT3-BOvoa_Lb.js";b();var w=`<?xml version="1.0" encoding="UTF-8"?>
...
`,O={key:0},k={class:`flex justify-center`},A={key:0,class:`flex flex-col justify-center text-center`},j=[`innerHTML`],M=[`innerHTML`],N={key:3,class:`flex flex-col justify-center text-center`},P={class:`text-xl mt-20`},F={key:1,class:`p-5`},I=f({__name:`screen`,props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(f){let b=t(),{t:I}=S(),L=h(`iApi`),R=p(),z=f,B=u(()=>b.status),V=u(()=>b.response),H=p(!1),U=v({}),W=v([]),G=v([]);l(()=>{b.status=`loading`,K(),W.push(L.event.on(n.LAYER_REMOVE,e=>{z.payload.layer?.uid===e.uid&&z.panel.close()})),G.push(m(()=>z.payload.layer.uid,(e,t)=>{e!==t&&(b.status=`loading`,K())}))}),o(()=>{W.forEach(e=>L.event.off(e)),G.forEach(e=>e())});let K=async()=>{if(H.value=z.payload.layer!==void 0&&!z.payload.layer.isRemoved,H.value){if(z.payload.type===`xml`){let e=await q(z.payload.url,[]);if(b.status=`success`,e?.firstElementChild){let t=document.createElement(`div`);t.appendChild(X(`${e.firstElementChild.outerHTML}`)),(z.payload.catalogueUrl||z.payload.url)&&t.appendChild(X(`<h5 class="text-xl font-bold mb-3">${I(`metadata.xslt.metadata`)}</h5>`)),z.payload.catalogueUrl&&t.appendChild(X(`<p><a style="color: blue;" href="${z.payload.catalogueUrl}" target="_blank">${I(`metadata.xslt.cataloguePage`)}</a></p>`)),t.appendChild(X(`<p><a style="color: blue;" href="${z.payload.url}" target="_blank">${I(`metadata.xslt.metadataPage`)}</a> (xml)</p>`)),b.response=t.outerHTML}}else if(z.payload.type===`html`){let e=await Y(z.payload.url);b.status=e.status,b.response=e.response}else if(z.payload.type===`md`){let e=await Y(z.payload.url);b.status=e.status,b.response=C(e.response,{async:!1})}}},q=async(t,n)=>{let r;if(r=z.payload.xmlType&&z.payload.xmlType===`DCAT`?L.language===`en`?E:D:L.language===`en`?w:T,r=r.replace(/\{\{([\w.]+)\}\}/g,(e,t)=>I(t)),!U[t]){let n=await Y(t);if(n.status===`error`){let t=n.reason?I(`metadata.notification.rawFallbackWithReason`,{reason:n.reason}):I(`metadata.notification.rawFallback`);L.notify.show(e.WARNING,t)}U[t]=n.response}return J(U[t],r,n)},J=(e,t,n)=>{let i=null;if(window.XSLTProcessor){let a=new window.XSLTProcessor,o=new DOMParser,s=o.parseFromString(e,`text/xml`),c=o.parseFromString(t,`text/xml`);a.importStylesheet(c),n&&n.forEach(e=>a.setParameter(``,e.key,e.value||``)),i=a.transformToFragment(s,document);let l={className:`underline text-blue-700 break-all`,target:`_blank`,validate:{url:e=>/^https?:\/\//.test(e)}},u=i.firstElementChild;u&&(z.payload.treatXmlAsMarkdown?u.innerHTML=C.parse(u.innerHTML,{async:!1}):u.innerHTML=r(u.innerHTML,l))}return i},Y=e=>new Promise(t=>{let n=new XMLHttpRequest;n.open(`GET`,e,!0),n.responseType=`text`,n.onload=()=>{n.status===200?t({status:`success`,response:n.response}):t({status:`error`,response:`Could not load results from remote service.`,reason:(n.statusText?`${n.status} ${n.statusText}`:`HTTP ${n.status}`)||void 0})},n.onerror=()=>{t({status:`error`,response:`Could not load results from remote service.`})},n.send()});function X(e){let t=document.createElement(`div`);return t.innerHTML=e,t}return(e,t)=>{let n=d(`panel-screen`);return i(),y(n,{panel:f.panel,ref_key:`el`,ref:R},{header:c(()=>[a(g(_(I)(`metadata.title`))+`: `+g(f.payload.layerName),1)]),content:c(()=>[H.value?(i(),s(`div`,O,[x(`div`,k,[B.value==`loading`?(i(),s(`div`,A,g(_(I)(`metadata.loading`)),1)):f.payload.type===`xml`&&B.value==`success`?(i(),s(`div`,{key:1,innerHTML:V.value,class:`flex flex-col justify-center max-w-full xml-content`},null,8,j)):(f.payload.type===`html`||f.payload.type===`md`)&&B.value==`success`?(i(),s(`div`,{key:2,innerHTML:V.value,class:`flex flex-col justify-center max-w-full metadata-view`},null,8,M)):(i(),s(`div`,N,[t[0]||=x(`img`,{src:`https://i.imgur.com/fA5EqV6.png`},null,-1),x(`span`,P,g(_(I)(`metadata.error`)),1)]))])])):(i(),s(`div`,F,[x(`span`,null,g(_(I)(`metadata.label.no.layer`)),1)]))]),_:1},8,[`panel`])}}});export{I as default};